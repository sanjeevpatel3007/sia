...
const getTodayEventsTool = tool({
  description:
    "Get all events scheduled for today from the user's calendar. Use this when the user asks about today's schedule, meetings, or appointments.",
...
const getUpcomingEventsTool = tool({
  description:
    "Get upcoming events from the user's calendar for the next few days. Use this when the user asks about their upcoming schedule, this week's meetings, or future appointments.",
  inputSchema: z.object({
    days: z
      .number()
      .optional()
  ...
  execute: async ({ days = 7 }: { days?: number }) => {
    try {
      ...
      if (events.length === 0) {
        return {
          success: true,
          message: `No events scheduled for the next ${days} day${
            days > 1 ? "s" : ""
          }.`,
          events: [],
        };
      ...
      return {
        success: true,
        message: `Found ${events.length} upcoming event${
          events.length > 1 ? "s" : ""
        } (showing first 10).`,
        events: formattedEvents,
      };
...
const searchCalendarEventsTool = tool({
  description:
    "Search for specific events in the user's calendar by keyword or topic. Use this when the user asks about specific meetings, appointments, or events by name.",
  inputSchema: z.object({
    query: z
      .string()
      .describe('Search query (e.g., "team meeting", "meditation", "yoga")'),
    maxResults: z
      .number()
      .optional()
      .default(10)
  ...
  execute: async ({
    ...
    try {
      const events = dummyCalendarService.searchEvents(
        query,
        "primary",
        maxResults
      );
...
const getEventsInRangeTool = tool({
  description:
    "Get events within a specific date range from the user's calendar. Use this when the user asks about events in a specific time period.",
  inputSchema: z.object({
    startDate: z
      .string()
      .describe('Start date in ISO format (e.g., "2025-01-15T00:00:00Z")'),
    endDate: z
      .string()
      .describe('End date in ISO format (e.g., "2025-01-20T23:59:59Z")'),
    maxResults: z
      .number()
      .optional()
      .default(50)
  ...
  execute: async ({
    ...
    try {
      const events = dummyCalendarService.getEvents(
        "primary",
        startDate,
        endDate,
        maxResults
...
const checkAvailabilityTool = tool({
  description:
    "Check if a specific time slot is available by comparing persona's calendar with Sama Studio schedule. Returns conflicts if the persona is busy or if the class time slot is not available at Sama Studio. Use this before suggesting booking times.",
  inputSchema: z.object({
    ...
    className: z
      ...
      .optional()
      .describe(
        'Optional: Specific class name to check at Sama Studio (e.g., "Yoga & Calm Mind Practice")'
      ),
  }),
  execute: async ({
    date,
    ...
    try {
      ...
      const samaConflicts = samaEvents.filter((event) => {
        ...
        return (
          eventStartTime.getHours() === startHour &&
      ...
      if (hasPersonaConflict && !hasSamaClass) {
        return {
          success: false,
          available: false,
          message: `${persona.name} is busy at ${timeSlot} (${
            personaConflicts[0].event
          }), and there's no ${
            className || "class"
          } at Sama Studio during this time.`,
        ...
        return {
          success: false,
          available: false,
          message: `${persona.name} is free, but there's no ${
            className || "class"
          } scheduled at Sama Studio at ${timeSlot} on ${date}.`,
          personaConflicts: [],
          samaClassesAvailable: [],
          suggestion:
            "Please check the Sama Studio schedule for available classes.",
        };
      } else {
        return {
          success: true,
...
const findAvailableSlotsTool = tool({
  description:
    "Find all available time slots for a persona on a specific date or range of days. Shows when the persona is free AND when Sama Studio has classes. Use this to suggest alternative booking times when there are conflicts.",
  inputSchema: z.object({
    ...
    days: z
      ...
      .optional()
      .default(7)
      .describe("Number of days to check (default: 7)"),
    className: z
      .string()
      .optional()
      .describe(
        'Optional: Filter for specific class type (e.g., "Yoga", "Pilates")'
      ),
  }),
  execute: async ({
    startDate,
    ...
    try {
      ...
      if (availableSlots.length === 0) {
        return {
          success: true,
          message: `No available time slots found for ${
            persona.name
          } in the next ${days} days${className ? ` for ${className}` : ""}.`,
          availableSlots: [],
        };
      }
      ...
      return {
        success: true,
        message: `Found ${availableSlots.length} available time slot${
